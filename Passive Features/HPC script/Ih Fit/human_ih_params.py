...
# ===========================================================================
#  5. Shift/scale-aware reference kinetics for the I_h FITTING campaign
#     (D-005 / D-007). Added 2026-09-22; everything above is unchanged so the
#     synthetic benchmark's smoke test keeps passing.
# ===========================================================================
# The two compiled mechanisms (mod/Ih.mod, mod/Ih_human.mod) expose the same
# three RANGE knobs:
#     vshift      (mV)  shift of BOTH curves; configuration, not fitted
#                       (Kalmbach 2018 = -20 on the Kole base)
#     vshift_minf (mV)  shift of the ACTIVATION curve only; fitted (Delta v_h)
#     tau_scale   (1)   multiplicative scale on mTau; fitted (kappa_tau)
# so that, for each fixed v (mV),
#     mInf(v) = mInf_base(v - vshift - vshift_minf)
#     mTau(v) = tau_scale * mTau_base(v - vshift)
# The functions below are the pure-Python reference of exactly that, used by
# ih_mechanism.balance_e_pas (Eq. 11.3 of the plan) and by smoke check S1.

IH_MECHANISMS: Tuple[str, ...] = ("Ih", "Ih_human")

IH_EHCN_DEFAULT_mV: Dict[str, float] = {
    "Ih": float(IH_KINETICS_RODENT_KOLE["ehcn_mV"]),        # -45.0
    "Ih_human": float(IH_KINETICS_HUMAN_RICH["ehcn_mV"]),   # -49.85
}


def _check_mechanism(mechanism: str) -> str:
    m = str(mechanism)
    if m not in IH_MECHANISMS:
        raise ValueError("unknown I_h mechanism %r; expected one of %s"
                         % (mechanism, IH_MECHANISMS))
    return m


def minf(v_mV: float, mechanism: str = "Ih_human", *,
         vshift_mV: float = 0.0, vshift_minf_mV: float = 0.0) -> float:
    """Steady-state activation of the SHIFTED curve, for each fixed v_mV.

    minf(v) = minf_base(v - vshift - vshift_minf). Dimensionless in (0, 1).
    """
    m = _check_mechanism(mechanism)
    u = float(v_mV) - float(vshift_mV) - float(vshift_minf_mV)
    if m == "Ih":
        return minf_kole_rodent(u)
    return minf_rich_human(u)


def mtau_ms(v_mV: float, mechanism: str = "Ih_human", *,
            vshift_mV: float = 0.0, tau_scale: float = 1.0,
            mtau_min_ms: float = 0.0) -> float:
    """Activation time constant of the SCALED curve [ms], for each fixed v_mV.

    mtau(v) = tau_scale * mtau_base(v - vshift), floored at mtau_min_ms
    (the floor is a documented deviation; 0 = published model).
    """
    m = _check_mechanism(mechanism)
    if not (float(tau_scale) > 0.0):
        raise ValueError("tau_scale must be > 0, got %r" % (tau_scale,))
    u = float(v_mV) - float(vshift_mV)
    if m == "Ih":
        tau = float(tau_scale) * mtau_kole_rodent_ms(u)
    else:
        tau = float(tau_scale) * mtau_rich_human_ms(u, mtau_min_ms=0.0)
    return max(tau, float(mtau_min_ms))


def ehcn_default_mV(mechanism: str = "Ih_human") -> float:
    """Reversal potential [mV] the compiled mechanism carries by default."""
    return IH_EHCN_DEFAULT_mV[_check_mechanism(mechanism)]


def v_half_mV(mechanism: str = "Ih_human", *,
              vshift_mV: float = 0.0, vshift_minf_mV: float = 0.0,
              v_lo: float = -200.0, v_hi: float = 0.0, tol: float = 1e-6) -> float:
    """Half-activation voltage [mV] of the shifted curve, by bisection.

    minf is monotone decreasing in v for both bases, so the root of
    minf(v) - 0.5 on [v_lo, v_hi] is unique. Pure Python (no scipy) so the
    smoke test has no extra dependency.
    """
    f_lo = minf(v_lo, mechanism, vshift_mV=vshift_mV, vshift_minf_mV=vshift_minf_mV) - 0.5
    f_hi = minf(v_hi, mechanism, vshift_mV=vshift_mV, vshift_minf_mV=vshift_minf_mV) - 0.5
    if f_lo * f_hi > 0:
        raise ValueError("v_half not bracketed on [%g, %g]" % (v_lo, v_hi))
    lo, hi = float(v_lo), float(v_hi)
    while hi - lo > tol:
        mid = 0.5 * (lo + hi)
        f_mid = minf(mid, mechanism, vshift_mV=vshift_mV, vshift_minf_mV=vshift_minf_mV) - 0.5
        if f_lo * f_mid <= 0:
            hi, f_hi = mid, f_mid
        else:
            lo, f_lo = mid, f_mid
...
def peak_mtau_shifted_ms(mechanism: str = "Ih_human", *,
                         vshift_mV: float = 0.0, tau_scale: float = 1.0,
                         v_lo: float = -160.0, v_hi: float = -30.0,
                         dv: float = 0.1) -> Tuple[float, float]:
    """(v_at_peak_mV, peak_mTau_ms) of the scaled/shifted mtau on a grid."""
    return peak_mtau_ms(
        lambda v: mtau_ms(v, mechanism, vshift_mV=vshift_mV, tau_scale=tau_scale),
        v_lo=v_lo, v_hi=v_hi, dv=dv)


__all__ += [
    "IH_MECHANISMS", "IH_EHCN_DEFAULT_mV", "minf", "mtau_ms",
    "ehcn_default_mV", "v_half_mV", "peak_mtau_shifted_ms",
]
