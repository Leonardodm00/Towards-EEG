...
ANNOTATION_LABEL_MAP = {
    ...
    '1002': 'Myelinated Fragment',
    '1003': 'Myelinated Fragment',
    '1004': 'Myelinated Fragment',
    '1005': 'Myelinated Fragment'
}
def reorient_fragment_nopr(df, new_root_id):
    """
...
def stitch_neuron_fragments_smart_nopr(neuron_id, skeleton_url):
    """
    1. Fetches skeleton directly from CloudVolume.
    2. Uses NetworkX to isolate disconnected topological fragments.
    ...
    try:
        ...
        skel = vol.skeleton.get(int(neuron_id))
    ...
    # --- 2. Identify Disconnected Components (Fragments) ---
    G = nx.Graph()
    G.add_nodes_from(df['id'])
    
    # Add valid edges (ignore p = -1 roots)
    edges = df[df['p'] != -1][['id', 'p']].values
    G.add_edges_from(edges)
    
    # Get isolated sub-trees and sort by size (largest first)
    ...
    # --- 4. Process and Stitch Fragments ---
    for i, comp_nodes in enumerate(components[1:]):
        frag_df = df[df['id'].isin(comp_nodes)].copy()
        if frag_df.empty: continue
        
        frag_name = f"Fragment {i+1}"
        
        # A. GEOMETRIC SEARCH (Against current Main Arbor)
        ...
        # C. RE-INDEXING (Add massive offset to prevent ID clashes)
...
def annotate_stitched_neuron_nopr(stitched_df, neuron_id, annotation_url=None):
    """
    Takes the stitched DataFrame, aligns it to the Cloud-Hosted Neuroglancer 
    Annotation space, and maps the closest labels to the skeleton nodes.
    ...
    Args:
        ...
        annotation_url (str): GCS Path to the precomputed annotations.
    ...
    try:
        # The AnnotationReader natively fetches from gs:// URLs
        ...
        # Fetch specific lookup for this neuron_id
...
def treat_distant_orphans(df_input_, soma_centroid, distance_threshold=25000, length_threshold=10000, connection_gap=200, plot_result=True):
    """
    Analyzes disconnected 'Orphan Roots' FAR from the soma.
    
    Logic:
    1. Identifies roots outside 'distance_threshold'.
    2. Traces arbor and calculates length.
    3. Checks if the arbor is ALREADY topologically connected to the Soma. 
       If yes, assigns dummy parent (-99999) to bypass multi-root errors.
    4. STRICT SPATIAL CHECK:
       - Gap > connection_gap: Deleted as Debris (regardless of length).
       - Gap <= connection_gap & Length < threshold: Deleted as Debris (Too short).
       - Gap <= connection_gap & Length >= threshold: Re-parented (Touching).
    """
    ...
    G_dir.add_edges_from(valid_edges)

    G_undir = nx.Graph()
    G_undir.add_edges_from(valid_edges)
    ...
    for _, row in orphans.iterrows():
        ...
        # B. Trace Full Arbor
        try:
            arbor_nodes = list(nx.descendants(G_dir, orphan_id))
        except nx.NetworkXError:
            arbor_nodes = []
        arbor_nodes.append(orphan_id)

        all_distant_orphan_nodes.update(arbor_nodes)
        ...
        # D. Check if ALREADY connected to Soma
        try:
            already_connected = nx.has_path(G_undir, orphan_id, 0)
        except nx.NodeNotFound:
            already_connected = False

        if already_connected:
            action = "Already Connected (Assigned Dummy Parent)"
            color = "orange"
            df.loc[df['id'] == orphan_id, 'p'] = -99999
            
            decision_log.append({
                'id': orphan_id, 'arbor_nodes': arbor_nodes, 'length': total_length,
                'gap': 0, 'action': action, 'color': color, 'link_coords': None
            })
            continue 

        # E. Find Closest Point to the Rest of the Arbor
        target_df = df[~df['id'].isin(arbor_nodes)]
        arbor_df = df[df['id'].isin(arbor_nodes)]
        
        if target_df.empty:
            continue

        tree = cKDTree(target_df[['x', 'y', 'z']].values)
        dists, indices = tree.query(arbor_df[['x', 'y', 'z']].values)

        best_idx_in_arbor = np.argmin(dists)
        min_dist = dists[best_idx_in_arbor]

        closest_orphan_node = arbor_df.iloc[best_idx_in_arbor]['id']
        closest_target_node = target_df.iloc[indices[best_idx_in_arbor]]['id']

        # --- F. STRICT DECISION LOGIC ---
        is_connected = min_dist <= connection_gap
        action_coords = None

        if not is_connected:
            action = f"Deleted (Debris - Gap {min_dist:.0f}nm > Limit)"
            color = "crimson"
            nodes_to_delete.update(arbor_nodes)
            
        elif total_length < length_threshold:
            action = f"Deleted (Debris - Too short: {total_length:.0f}nm)"
            color = "crimson"
            nodes_to_delete.update(arbor_nodes)
            
        else:
            action = "Re-parented (Touching & Valid Length)"
            color = "cyan"
            action_coords = (
                arbor_df.iloc[best_idx_in_arbor][['x','y','z']].values,
                target_df.iloc[indices[best_idx_in_arbor]][['x','y','z']].values
            )

            # Reroot and Connect
            df = manual_reroot(df, closest_orphan_node, arbor_nodes)
            df.loc[df['id'] == closest_orphan_node, 'p'] = closest_target_node
            
            # Update map for future loops and add edge to undirected graph
            node_map = df.set_index('id').to_dict('index') 
            G_undir.add_edge(closest_target_node, closest_orphan_node)

        decision_log.append({
            'id': orphan_id, 'arbor_nodes': arbor_nodes, 'length': total_length,
            'gap': min_dist, 'action': action, 'color': color, 'link_coords': action_coords
        })
    ...
    def plot_distant_decisions(log, full_node_map, orphan_set):
        ...
        main_ids = all_ids - orphan_set
        main_x, main_y, main_z = [], [], []

        for nid in main_ids:
            node = full_node_map[nid]
            pid = node['p']
            # Treat the dummy ID as a break in the drawing lines
            if pid != -1 and pid != -99999 and pid in full_node_map:
        ...
        fig.add_trace(go.Scatter3d(
            ...
            mode='lines', line=dict(color='lightgrey', width=2), opacity=0.5, name='Main Skeleton'
        ))
        ...
        for entry in log:
            ...
            # Draw Re-parented connections
            if entry['link_coords'] is not None:
                p1, p2 = entry['link_coords']
                fig.add_trace(go.Scatter3d(
                    x=[p1[0], p2[0]], y=[p1[1], p2[1]], z=[p1[2], p2[2]],
                    mode='lines+markers', 
                    line=dict(color='cyan', width=4, dash='dash'),
                    marker=dict(size=4, color='cyan'),
                    name=f"Connection (Gap: {entry['gap']:.0f}nm)", showlegend=False
                ))

        # C. Soma
        fig.add_trace(go.Scatter3d(
            x=[soma_centroid[0]], y=[soma_centroid[1]], z=[soma_centroid[2]],
            mode='markers', marker=dict(size=8, color='black', symbol='diamond'), name='Soma Center'
        ))

        fig.update_layout(title="Distant Orphan Analysis & Stitching", scene=dict(aspectmode='data', bgcolor='white'), height=800)
        fig.show()

    # --- 4. EXECUTE PLOT ---
    if plot_result and decision_log:
        plot_distant_decisions(decision_log, node_map, all_distant_orphan_nodes)
...
def annotate_stitched_neuron(stitched_df, neuron_id, annotation_url=None):
    """
...
def validate_reconstruction_quality_RAMopt_(df, com_distance_threshold=50000, min_arbor_length=0, output_dir="QA_Reports", max_ram_gb=8.0):
    """
    Evaluates the quality of the final reconstructed neuron.
    Uses A Priori RAM estimation to prevent Colab crashes.
    """
 ...
 # --- A PRIORI RAM ESTIMATION (NON-LINEAR) ---
    N = len(df)
    
    # 1. Base structural overhead for NetworkX and Navis internal dicts (~20 KB per node)
    base_ram_bytes = N * 20_000 
    
    # 2. Spatial complexity overhead for DBSCAN and KDTree matrices.
    #    Neighborhood queries scale exponentially with point cloud size (N^1.5)
    spatial_ram_bytes = (N ** 1.5) * 50
    estimated_peak_gb = (base_ram_bytes + spatial_ram_bytes) / (1024 ** 3)
    
    print('ESTIMATED RAM')
    print(estimated_peak_gb)
    if estimated_peak_gb > max_ram_gb:
        print(f"[FAIL] FAIL: Predicted RAM limit exceeded.")
        print(f"  - Neuron has {N:,} nodes.")
        print(f"  - Estimated peak RAM for QA operations: {estimated_peak_gb:.2f} GB (Limit: {max_ram_gb} GB).")
        print("  - Skipping QA check to prevent Colab from crashing.")
        return False
    # -------------------------------
    ...
    # --- CHECK 4: At Least One Axon Arbor ---
    axon_branches = 0
    ...
    # --- CHECK 6: Total Arbor Length ---
    segments = df_clean.merge(
        df_clean[['id', 'x', 'y', 'z']], 
        left_on='p', 
        right_on='id', 
        suffixes=('', '_parent')
    )
    total_length_nm = np.sqrt(
        (segments['x'] - segments['x_parent'])**2 + 
        (segments['y'] - segments['y_parent'])**2 + 
        (segments['z'] - segments['z_parent'])**2
    ).sum()

    if total_length_nm < min_arbor_length:
        is_valid = False
        reasons.append(f"Size Error: Total arbor length ({total_length_nm:,.1f} nm) is below the minimum threshold ({min_arbor_length:,.1f} nm).")

    # --- TERMINAL OUTPUT ---
    print(f"\n QA Result: {'[OK] PASS' if is_valid else '[FAIL] FAIL'}")
    if not is_valid:
    ...
    return is_valid
def validate_reconstruction_quality(df, com_distance_threshold=50000, min_arbor_length=0, output_dir="QA_Reports",max_ram_gb=8.0):
    """
    ...
    Checks:
      ...
      4. Presence of >= 1 Axon arbor (at least one primary branch leading to an axon).
      5. Soma is centered within com_distance_threshold (nm) of the entire arbor.
      6. Total arbor length is greater than or equal to min_arbor_length (nm).
    """
    print("\n Running Quality Assurance Check...")
