# -*- coding: utf-8 -*-
"""Final  Get_comparments_h10.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ARlFopOry1VJXLkCQKxR3WJtMemCejpW
"""
...
SKELETON_PATH = "gs://h01-release/data/20210601/c3/skeletons"
...
ANNOTATION_LABEL_MAP = {
    ...
    '1001': 'Myelinated Axon', '1002': 'Myelinated Fragment',
    '1003': 'Myelinated Fragment', '1004': 'Myelinated Fragment',
    '1005': 'Myelinated Fragment'
}

def extract_skeleton_data(neuron_id):
    """
    1. Loads Annotation Cloud (Reference) directly from GCS.
    2. Loads SWC Skeleton (Target) directly from GCS.
    ...
    try:
        # AnnotationReader natively supports reading directly from gs:// URLs
    ...
    # Strip the 'gs://' prefix just for the search pattern, as gcsfs.glob expects raw paths
    clean_skeleton_path = SKELETON_PATH.replace("gs://", "")
    search_pattern = f"{clean_skeleton_path}/{neuron_id}*.swc"
    ...
    for f in files:
        try:
            # Re-attach 'gs://' so pandas knows to stream it from Google Cloud
            gs_path = f"gs://{f}"
            df = pd.read_csv(gs_path, delim_whitespace=True, comment='#',
...
# NOTE: We add the "precomputed://" prefix so CloudVolume knows the file format
...
def extract_skeleton_data_nopr(neuron_id):
    """
    1. Loads Annotation Cloud (Reference) directly from GCS via Neuroglancer library.
    2. Loads Skeleton (Target) directly from GCS via CloudVolume API.
    3. Aligns Skeleton to Cloud.
    4. Finds nearest Annotation for each Skeleton node (With 5um Max Dist).
    """
    print(f"📥 Extracting and Mapping data from Cloud for Neuron {neuron_id}...")
    ...
    try:
        ...
        # Pull the binary skeleton down from the cloud
        skel = vol.skeleton.get(int(neuron_id))

        # Convert the binary format to a standard SWC string in memory
        swc_string = skel.to_swc()

        # Read the SWC string directly into Pandas
        swc_df = pd.read_csv(io.StringIO(swc_string), delim_whitespace=True, comment='#',
                             names=['id', 'type_code', 'x', 'y', 'z', 'radius', 'parent'])
    ...
    # Standardize parent column to match Rec_Utility expectations
    final_df.rename(columns={'parent': 'p'}, inplace=True)
...
# EXECUTE
# ==========================================
# Ensure 'df' exists from the previous steps
...
# ==========================================
if 'df' in locals() and df is not None:
    visualize_comparison_lines(neuron_id=neuron_id, df_processed=df)
...
"""## PLOT SKELETON AND ANNOTATION POINTS"""
...
# ==========================================
if 'df' in locals() and df is not None:
    soma_center, soma_ids = find_stable_soma_centroid(df)
...
if 'df' in locals() and 'soma_center' in locals() and soma_center is not None:
    ...
    # 1. Plot ONLY the skeleton of the soma
    # Ensure you pass arguments in this order: df, ids, center
    plot_soma_skeleton(df, soma_ids, soma_center)
...
def find_and_plot_radial_exits(df, centroid, soma_ids):
    """
    1. Identifies nodes where the skeleton leaves the Soma cluster.
    2. Calculates the radial distance of these points from the centroid.
    3. Visualizes the exits with radial vectors.
    """
    if df is None or centroid is None: return

    print("Search for Radial Exit Points...")

    # --- 1. HANDLE DUPLICATE IDS (Robust Lookup) ---
    df_safe = df.copy()
    if 'file_source' not in df_safe.columns: df_safe['file_source'] = 'single_file'

    # Create Unique IDs
    df_safe['uid'] = df_safe['file_source'].astype(str) + "_" + df_safe['id'].astype(str)

    # Map Parent IDs to UIDs
    pid_col = 'p' if 'p' in df_safe.columns else 'parent'
    df_safe['p_uid'] = df_safe['file_source'].astype(str) + "_" + df_safe[pid_col].astype(str)

    # Build Fast Lookup for Node Info
    nodes = df_safe.set_index('uid').to_dict('index')

    # --- 2. FIND TRANSITIONS (Soma -> Non-Soma) ---
    exits = []

    for _, row in df_safe.iterrows():
        node_id = row['id']
        p_id = int(row[pid_col])
        p_uid = row['p_uid']

        # LOGIC:
        # 1. Parent MUST be in the Stable Soma Cluster.
        # 2. Child (Current Node) MUST NOT be in the Soma Cluster.
        if p_id in soma_ids and node_id not in soma_ids:

            # Calculate Radial Distance
            pos = np.array([row['x'], row['y'], row['z']])
            dist = np.linalg.norm(pos - centroid)

            # Get the type of the branch starting here
            b_type = row['annotated_type'] if 'annotated_type' in row else 'Unknown'
    ...
    # A. Plot Soma Cloud (Context)
    soma_pts = df_safe[df_safe['id'].isin(soma_ids)]
    fig.add_trace(go.Scatter3d(
        x=soma_pts['x'], y=soma_pts['y'], z=soma_pts['z'],
        mode='markers', marker=dict(size=3, color='green', opacity=0.15),
        name='Soma Cloud'
    ))
    ...
    # C. Plot Exits + Radial Lines
    # Colors for different branch types
    CMAP = {'Axon': 'red', 'AIS': 'gold', 'Dendrite': 'blue',
            'Astrocyte': 'purple', 'Myelinated Axon': 'darkred'}

    for b_type, data in exit_df.groupby('type'):
        color = CMAP.get(b_type, 'orange')

        # 1. The Exit Node Marker
        fig.add_trace(go.Scatter3d(
        ...
        # 2. Radial Lines (Centroid -> Exit)
        # Visually proves the "Radial" aspect
        x_lines, y_lines, z_lines = [], [], []
        for _, row in data.iterrows():
...
# EXECUTE
# ==========================================
# Requires 'soma_center' and 'soma_ids' from the previous DBSCAN step
if 'df' in locals() and 'soma_center' in locals() and soma_center is not None:
...
def plot_swc_components(neuron_id, skeleton_path):
    """
    Finds all SWC files for a given neuron_id and plots them in 3D.
    Each file is assigned a unique color to visualize component separation.

    Args:
        neuron_id (str/int): The root ID of the neuron.
        skeleton_path (str): Directory containing the SWC files.
    """
    # 1. Find files
    pattern = os.path.join(skeleton_path, f"{neuron_id}*.swc")
    files = glob.glob(pattern)
    ...
    # Get a list of distinct colors (Plotly qualitative palette)
    colors = pc.qualitative.Dark24 * 10  # Repeat to ensure enough colors
    ...
    for i, file_path in enumerate(files):
        ...
        try:
            ...
            # 3. Build fast lookup for coordinates
            # We need this to draw lines from Child -> Parent
            coords = df.set_index('id')[['x', 'y', 'z']].to_dict('index')

            # 4. Construct Line Segments
            # Plotly approach: Single trace with segments separated by None
            x_lines, y_lines, z_lines = [], [], []

            for _, row in df.iterrows():
                parent_id = int(row['p'])

                # Draw connection only if parent is not root (-1) and parent exists in file
                if parent_id != -1 and parent_id in coords:
                    parent_node = coords[parent_id]

                    x_lines.extend([row['x'], parent_node['x'], None])
                    y_lines.extend([row['y'], parent_node['y'], None])
                    z_lines.extend([row['z'], parent_node['z'], None])

            # 5. Add Trace for this File
            fig.add_trace(go.Scatter3d(
                x=x_lines, y=y_lines, z=z_lines,
                mode='lines',
                line=dict(color=color, width=3),
                name=file_name, # Shows up in Legend
                hoverinfo='name'
            ))
    ...
    # --- Layout ---
    fig.update_layout(
        title=f"Component Breakdown: {neuron_id} ({len(files)} files)",
        scene=dict(
            xaxis_title='X (nm)', yaxis_title='Y (nm)', zaxis_title='Z (nm)',
            aspectmode='data', # Maintains biological proportions
            bgcolor='white'
        ),
        legend=dict(title="SWC Files"),
        height=900,
...
plot_swc_components(neuron_id,SKELETON_PATH)
...
plot_merged_neuron(Stiched_neuron)

"""# Same but for NOT proofread neurons"""
...
"""## Check for unconnected components which, however, are likely to belong to the same branch"""
...
"""## ADD ANNOTATION TO THE STICHED SKELETON"""
...
# 2. Run the annotation mapping on the result
# (Replace '12345' with your actual neuron ID variable)
final_df = annotate_stitched_neuron(Stiched_neuron, neuron_id=neuron_id)
...
def annotate_stitched_neuron_nopr(stitched_df, neuron_id, annotation_url=ANNOTATION_URL):
    """
...
# 2. Run the annotation mapping on the result
# (Replace '12345' with your actual neuron ID variable)
final_df = annotate_stitched_neuron_nopr(Stiched_neuron, neuron_id=neuron_id)

"""## PLOT STICHED AND ANNOTATED SKELETON"""
...
# --- USAGE ---
plot_annotated_neuron(final_df, title=f"Neuron {neuron_id} Annotations")
...
plot_voxel_vs_physical(final_df,final_df_nm)
...
# ==========================================
if 'df' in locals() and df is not None:
    soma_center, soma_ids = find_stable_soma_centroid(final_df, min_samples=50, step_eps=250, max_eps=40000, REQUIRED_STABILITY= 10)
...
if 'df' in locals() and 'soma_center' in locals() and soma_center is not None:
    ...
    # 1. Plot ONLY the skeleton of the soma
    # Ensure you pass arguments in this order: df, ids, center
    plot_soma_skeleton(final_df, soma_ids, soma_center)

#################### JUST FOR NON PROOFREAD NEURONS
...
# USAGE
# ========================
plot_soma_points(final_df_nm, soma_ids, title="Expanded Soma Cluster")

"""## Find exit branches (new algorithm)"""
...
#     """
#     Finds the first NON-SOMA node for every branch crossing the sphere.
#     These nodes will serve as the connection points to the new Virtual Root.
...
#         comp_list = list(comp)
#         # Pick point closest to centroid in this cluster
...
#             # Walk path to find transition: Soma -> Non-Soma
#             for i in range(len(path) - 1):
#                 curr_node = path[i]
#                 next_node = path[i+1] # <--- THIS IS THE KEY (Child Node)

#                 curr_type = node_map[curr_node].get('annotated_type', 'Unknown')
#                 next_type = node_map[next_node].get('annotated_type', 'Unknown')

#                 # Check labels (Handling both string 'Soma' and int '1' or '3')
...
#                         'id': next_node,  # <--- SAVE THE CHILD (Branch Start)
...
#     # Add Coords for Plotting & Collapse Function
...
# unique_exits = find_exits_by_sphere_intersection(final_df_nm, soma_center, radius=10000, tolerance=5000)

"""## Alternative exit nodes search"""
...
unique_exits,final_df_nm = find_exits_by_sphere_intersection(final_df, soma_center, radius=40000, tolerance=10000)

"""# Connect fragmented branches"""
...
#     """
#     Manually re-orients a specific set of nodes (component) so that 'root_id'
#     becomes the new root (parent = -1).

#     Logic:
#     1. Builds an undirected graph of the component.
#     2. Performs BFS starting from 'root_id'.
#     3. Rebuilds parent-child relationships based on the BFS traversal order.
#     """
#     # Filter for the relevant rows
#     subset = df[df['id'].isin(component_ids)].copy()

#     # Build Undirected Graph of this component
#     G_undirected = nx.Graph()
#     G_undirected.add_nodes_from(subset['id'])

#     # Add existing edges (ignoring -1 parents)
#     edges = subset[subset['p'] != -1][['id', 'p']].values
#     G_undirected.add_edges_from(edges)

#     # Traverse from the new root to determine new hierarchy
#     # BFS guarantees we process level by level
#     bfs_tree = nx.bfs_tree(G_undirected, source=root_id)

#     # Create the new parent mapping based on the BFS tree
#     # In bfs_tree, an edge (u, v) means u is parent of v
#     new_parents = {}
...
#     # We map the 'id' to the 'new_parent'
#     # Any node not in the tree (orphan within component) keeps its old parent or stays -1
#     df.loc[df['id'].isin(new_parents.keys()), 'p'] = df['id'].map(new_parents)

#     return df

# def stitch_floating_ends(df, reference_point, threshold=2000):
#     """
#     Connects floating end points to nearby segments using KDTree and Manual Rerooting.
#     NO NAVIS dependency.
#     """
#     if df is None or df.empty:
#         print("❌ DataFrame is empty.")
...
#     # --- 1. BUILD GRAPH FOR TOPOLOGY ---
#     # We need a graph to find connected components (fragments)
#     G_full = nx.Graph()
#     G_full.add_nodes_from(df_stitched['id'])
#     valid_edges = df_stitched[df_stitched['p'] != -1][['id', 'p']].values
#     G_full.add_edges_from(valid_edges)

#     # --- 2. IDENTIFY LEAVES (Tips) ---
#     all_ids = set(df_stitched['id'])
#     parent_ids = set(df_stitched['p'])
...
#     idx_to_id = dict(enumerate(df_stitched['id']))

#     connections = []

#     for leaf_id in leaf_ids:
#         leaf_pos = np.array([node_map[leaf_id]['x'], node_map[leaf_id]['y'], node_map[leaf_id]['z']])

#         # Find neighbors
#         dists, indices = tree.query(leaf_pos, k=10, distance_upper_bound=threshold)
...
#             target_id = idx_to_id[idx]

#             if target_id == leaf_id: continue
#             if target_id == node_map[leaf_id]['p']: continue # Don't stitch to own parent

#             # CRITICAL: Do not connect if they are already in the same fragment
#             # This prevents loops (e.g., axon tip curling back to touch itself)
#             if nx.has_path(G_full, leaf_id, target_id):
#                 continue
...
#         if best_target:
#             connections.append((leaf_id, best_target, min_dist))
...
#     for id_a, id_b, dist in connections:
#         # Re-check connectivity (fragments might have merged in previous iteration)
#         if nx.has_path(G_full, id_a, id_b):
#             continue

#         # Determine Hierarchy based on Soma Distance
#         pos_a = np.array([node_map[id_a]['x'], node_map[id_a]['y'], node_map[id_a]['z']])
#         pos_b = np.array([node_map[id_b]['x'], node_map[id_b]['y'], node_map[id_b]['z']])

#         dist_a = np.linalg.norm(pos_a - reference_point)
#         dist_b = np.linalg.norm(pos_b - reference_point)

#         if dist_a < dist_b:
#             parent_id, child_id = id_a, id_b # A is closer (Proximal)
#         else:
#             parent_id, child_id = id_b, id_a # B is closer (Proximal)

#         try:
#             # 1. Get the component (fragment) containing the Child
#             # We must reroot this entire fragment so the Child becomes the root
#             component_ids = list(nx.node_connected_component(G_full, child_id))

#             # 2. Manual Reroot
#             # Flips arrows in df_stitched for all nodes in component_ids
#             df_stitched = manual_reroot(df_stitched, child_id, component_ids)

#             # 3. Connect
#             # Set the Child (now a root) to point to the Parent
#             df_stitched.loc[df_stitched['id'] == child_id, 'p'] = parent_id

#             # 4. Update Graph (So future checks know they are merged)
#             G_full.add_edge(child_id, parent_id)
...
#     # --- 5. PLOT ---
#     if count > 0:
#         plot_stitched_skeleton(df_stitched, new_links_coords, title="Stitched End-Points (Manual Reroot)")
...
#         if pid != -1 and pid in coords:
#             p = coords[pid]
#             x_lines.extend([row['x'], p['x'], None])
#             y_lines.extend([row['y'], p['y'], None])
#             z_lines.extend([row['z'], p['z'], None])

#     fig.add_trace(go.Scatter3d(
#         x=x_lines, y=y_lines, z=z_lines,
#         mode='lines', line=dict(color='silver', width=1),
...
#             name=f'New Merges ({len(new_links)})'
#         ))

#     fig.update_layout(title=title, scene=dict(aspectmode='data', bgcolor='white'), height=800)
#     fig.show()



# df_stitched = stitch_floating_ends(final_df, soma_center,8000)
...
collapsed_neuron = collapse_soma_to_root(
    df_=final_df_nm,
    soma_ids=soma_ids,       # From DBSCAN Step
    centroid=soma_center,    # From DBSCAN Step
    exit_df=unique_exits      # From Radial Exits Step
)

# # Verify
# print(collapsed_neuron.head())
...
plot_collapsed_neuron(collapsed_neuron)
...
df_orphantreated_proximal = treat_orphan_roots(collapsed_neuron, soma_centroid=soma_center, distance_threshold=25000, length_threshold=10000, plot_result=True)
...
df_collapsed_stiched = highlight_and_stitch_points(df_orphantreated, soma_center, threshold=100, min_graph_hops=150, stitch=True)

"""## Replot after stiches"""

plot_collapsed_neuron(df_collapsed_stiched)

"""## Check consistency for reroting
...
# --- USAGE ---
plot_soma_and_roots(df_collapsed_stiched)

print(collapsed_neuron.loc[collapsed_neuron['id'] == 0, 'p'])
print(collapsed_neuron[collapsed_neuron['id'] == 0][['p']])
...
# ==========================================
plot_final_neuron(final_oriented_df, title="Final Corrected Neuron")
...
plot_skeleton_comparison(collapsed_neuron, Labled_propagated)

output_folder = "/content/drive/MyDrive/Colab Notebooks/Reconstructed neurons"

# Construct filename: "reconstructed_neurons/neuron_12345.csv"
filename = f"{output_folder}/neuron_{neuron_id}.csv"
