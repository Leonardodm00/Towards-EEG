...
elements = [
    "L23_exc", "L23_inh", "L4_exc", "L4_inh", "L4_ss",
    "L5_exc", "L5_inh", "L6_exc", "L6_inh"
]

# Method 1: Iterating and printing
for item in elements:
    # split('_') turns "L23_exc" into ["L23", "exc"]
    # [-1] grabs the last element in that list
    suffix = item.split('_')[-1]
    print(f"Element: {item} | Suffix: {suffix}")

# Method 2: Creating a new list of suffixes (List Comprehension)
suffixes = [item.split('_')[-1] for item in elements]

print("\nAll suffixes:", suffixes)
...
def calculate_spanning_probabilities_jitter_comparison(neuron_ids, metadata_filepath, input_dir, rnd_angles = True,num_angles = 20,voxel_size=10.0, grid_extent=1000.0, k_neighbors=3):
    """
...
def debug_snapping_accuracy_3d(
    ...
    metadata_filepath,
    post_soma_pos,
    cell_mtypes,           # <--- NEW: Passed in to identify the source of each connection
    k_neighbors=3
):
    """
    ...
    if not root_rows.empty:
        ...
        # Smooth, elegant slate-grey arbor
        fig.add_trace(go.Scatter3d(
            x=skel_x, y=skel_y, z=skel_z,
            mode='lines',
            line=dict(color='#4A5568', width=2.0),
            opacity=0.4,
            name='Post-Synaptic Arbor',
            hoverinfo='none'
        ))
    ...
    # ==========================================
    # 3. GROUP SYNAPSES BY PRE-SYNAPTIC M-TYPE
    # ==========================================
    # Dictionary to hold the coordinates sorted by their source type
    categorized_coords = {}

    for pre_idx, lfpy_indices in final_mapped_synapses_dict.items():
        # Look up the mtype of the pre-synaptic cell
        try:
            source_mtype = str(cell_mtypes[pre_idx])
        except IndexError:
            source_mtype = 'Unknown'

        # Initialize the coordinate lists for this specific mtype if it doesn't exist
        if source_mtype not in categorized_coords:
            categorized_coords[source_mtype] = {'x': [], 'y': [], 'z': []}

        # Look up the 3D coordinates for the snapped LFPy indices
        for l_idx in lfpy_indices:
            match = mapped_df[mapped_df['lfpy_idx'] == l_idx]
            if not match.empty:
                loc_x = match.iloc[0]['x'] + post_soma_pos[0]
                loc_y = match.iloc[0]['y'] + post_soma_pos[1]
                ...
                categorized_coords[source_mtype]['x'].append(loc_x)
                categorized_coords[source_mtype]['y'].append(loc_y)
                categorized_coords[source_mtype]['z'].append(loc_z)

    # ==========================================
    # 4. PLOT EACH PRE-SYNAPTIC GROUP
    # ==========================================
    # Specific colors for Virtual Cells
    special_colors = {
        'TC': '#F72585',      # Vibrant Magenta/Pink
        'BG_exc': '#FF9F1C',  # Glowing Neon Orange
        'BG_inh': '#00F5D4',  # Electric Cyan/Mint
    }

    # A distinct color palette for the intracortical cells (L23_PC, L4_SS, etc.)
    # We use Plotly's built in 'Vivid' sequence for dynamic assignment
    cortical_palette = px.colors.qualitative.Vivid + px.colors.qualitative.Pastel
    color_index = 0

    for source_mtype, coords in categorized_coords.items():
        if len(coords['x']) == 0:
            continue

        # Assign color
        if source_mtype in special_colors:
            marker_color = special_colors[source_mtype]
            marker_symbol = 'diamond'
            marker_size = 5
            zorder_offset = 2 # Bring virtual to front
        else:
            marker_color = cortical_palette[color_index % len(cortical_palette)]
            marker_symbol = 'circle'
            marker_size = 4
            color_index += 1
            zorder_offset = 1

        # Add the scatter trace for this specific M-Type
        fig.add_trace(go.Scatter3d(
            ...
            name=f'From {source_mtype} ({len(coords["x"])} syn)'
        ))

    # Add Post-Soma Marker (Glowing Gold Core)
    fig.add_trace(go.Scatter3d(
        x=[post_soma_pos[0]], y=[post_soma_pos[1]], z=[post_soma_pos[2]],
        mode='markers',
        marker=dict(size=10, color='#FFD166', symbol='circle', line=dict(color='#FFFFFF', width=2), opacity=1.0),
        name='Post-Soma Position'
    ))

    # ==========================================
    # 5. ENVIRONMENT AND LAYOUT STYLING
    # ==========================================
    axis_style = dict(
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>Synaptic Inputs by Source Identity</b><br><sup>Neuron {post_neuron_id}</sup>",
...
# Mock pre-synaptic partners (e.g., cell 101 gives 5 synapses, cell 102 gives 5 synapses)
Cell_afferences = {
    0: np.array([[101, 5], [102, 5],[103, 5],[110,9],[111,4],[112,3]])
}
pre_partners_matrix = Cell_afferences[cellindex]
