...
def plot_synapse_layer_distribution_log(mtypes, synapse_dict, filename='synapse_layer_distribution_log.png'):
    ...
    Uses a grouped bar chart with a logarithmic Y-axis for accurate magnitude comparison.
    """

    ordered_post = ['L1', 'L23', 'L4', 'L5', 'L6']
    ordered_pre = ['L1', 'L23', 'L4', 'L5', 'L6', 'th']
    ...
    for post_id, matrix in synapse_dict.items():
        ...
        post_mtype = str(mtypes[post_id])
        if post_mtype == 'TC' or post_mtype == 'nan':
            continue
        ...
        for row in matrix:
            ...
            # Map the pre-synaptic layer
            pre_layer = 'th' if pre_mtype == 'TC' else pre_mtype.split('_')[0]

            if pre_layer in counts[post_layer]:
                counts[post_layer][pre_layer] += num_syn

    # 2. Plotting the Grouped Bar Chart
    fig, ax = plt.subplots(figsize=(12, 7)) # Made slightly wider to accommodate grouped bars
    ...
    for i, pre_layer in enumerate(ordered_pre):
        ...
        ax.bar(offset, values, width, label=f'From {pre_layer}', color=colors[i], edgecolor='white', linewidth=0.5)
...
multapse_dict = extract_multapses(connectomics_path,post_to_pre_,mtypes_,mtype_fast_lookup)


plot_synapse_layer_distribution_log(mtypes_, multapse_dict, filename='synapse_layer_distribution.png')