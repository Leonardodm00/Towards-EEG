"""test_stage0_plots -- smoke test for stage0_plots.

RUN
---
    python3 test_stage0_plots.py
    python3 test_stage0_plots.py --save /tmp/figs      # also write PNGs
...
import stage0_plots as s0p                 # noqa: E402
...
def test_returns_figures():
    ...
    cases = [
        ...
        ("stage0_batch", s0p.stage0_batch(s0)), 
    ]
...
def test_empty_inputs():
    ...
    for name, fn, arg in (
            ...
            ("truncation_diagnostics", s0p.truncation_diagnostics,
             {"e": empty_trunc}),
            ("stage0_batch", s0p.stage0_batch, pd.DataFrame())):
        try:
            fig = fn(arg)
...
def test_suspect_radii_path():
    ...
    s0 = make_stage0_df(n=6)
    fig = s0p.stage0_batch(s0)
...
def main():
    print("stage0_plots smoke test  (%s)" % s0p.MODULE_VERSION)
