"""stage0_plots -- figures for the S1 stage-0 quantities.
...
MODULE_VERSION = "stage0_plots-1.0.0"
...
# 6. Batch rollup                                                              #
# --------------------------------------------------------------------------- #
def stage0_batch(stage0_df):
    """Per-cell rollup of the stage-0 quantities across the bank.
    ...
    """
    fig, axes = plt.subplots(2, 2, figsize=(12, 7.5))
    if stage0_df is None or len(stage0_df) == 0:
        return _empty(fig, "no stage-0 rows")

    df = stage0_df.reset_index(drop=True)
    n = len(df)
    x = np.arange(n)
    ...
    fig.suptitle("stage-0 across the bank, n = %d cells%s"
